...
struct Monitor
{
    ...
    {
        return (xpos >= x + margin && xpos < (x + w - margin) &&
                ypos >= y + margin && ypos < (y + h - margin));
    }
};
...
void confinePointer(const Monitor *mon)
{
    if (pointerConfined == 0)
    {
        ...
        XGrabPointer(display, mon->inputWindow, false,
                     ...
                     GrabModeAsync, GrabModeAsync, mon->inputWindow, None,
                     lastPtrMoveX11Time);
        ...
        int x, y, win_x, win_y;
        unsigned int maskDummy;
        XQueryPointer(display, rootWindow, &rootWindow, &childWnd, &x, &y,
        ...
        std::cout << "Confined pointer to " << mon->x << "x" << mon->y << "-"
                  << mon->w << "x" << mon->h << std::endl;
    }
}
